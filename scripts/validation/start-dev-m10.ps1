# GlyphDeck validation harness - start dev servers (M10)
...
$fp = "5173"
Write-Host "=== GlyphDeck M10 — Start Dev ==="
...
Start-Job -Name "glyphdeck-m10-backend" -ScriptBlock { param($exe,$log,$port) $env:GLYPHDECK_PORT=$port; & $exe >> $log 2>&1 } -Arg $bkExe,$backendLog,$bp | Out-Null
