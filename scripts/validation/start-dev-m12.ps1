# GlyphDeck validation harness - start dev servers (M12)
...
$fp = "5173"
Write-Host "=== GlyphDeck M12 — Start Dev ==="
...
Start-Job -Name "glyphdeck-m12-backend" -ScriptBlock { param($exe,$log,$port) $env:GLYPHDECK_PORT=$port; & $exe >> $log 2>&1 } -Arg $bkExe,$backendLog,$bp | Out-Null
...
Start-Job -Name "glyphdeck-m12-frontend" -ScriptBlock { param($dir,$log) Set-Location $dir; & cmd.exe /c "npm.cmd run dev" >> $log 2>&1 } -Arg (Join-Path $repoRoot "web"),$frontendLog | Out-Null
