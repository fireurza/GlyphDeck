# GlyphDeck validation harness — start dev servers as hidden background jobs
# Usage: .\scripts\validation\start-dev.ps1
...
Write-Host "=== GlyphDeck Validation Harness — Start Dev ==="
...
$backendJob = Start-Job -Name "glyphdeck-backend" -ScriptBlock {
...
for ($i = 1; $i -le $maxAttempts; $i++) {
    try {
        ...
        if ($resp.StatusCode -eq 200) {
            ...
            # Extract PID from the job's child process (go run spawns child).
            $backendPID = try {
                (Get-Job -Name "glyphdeck-backend" | Receive-Job -Keep 2>$null)
...
$frontendJob = Start-Job -Name "glyphdeck-frontend" -ScriptBlock {
...
Write-Host "Backend : http://127.0.0.1:${backendPort}  (job: glyphdeck-backend)"
Write-Host "Frontend: http://localhost:${frontendPort}   (job: glyphdeck-frontend)"
