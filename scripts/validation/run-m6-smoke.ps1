# GlyphDeck validation harness -- Milestone 6 smoke test (Usage tab)
# Usage: .\scripts\validation\run-m6-smoke.ps1
# Starts M6 servers, runs Playwright Usage tab smoke test, stops servers.
...
Write-Host "=== GlyphDeck M6 Usage Tab Smoke Test ==="
...
# --- Start servers ---
Write-Host ""
Write-Host "[harness] Starting M6 dev servers..."
$startScript = Join-Path $scriptDir "start-dev-m6.ps1"
& $startScript
Write-Host ""
...
# --- Run Playwright ---
$playwrightScript = Join-Path $scriptsDir "m6-smoke.cjs"
Write-Host ""
Write-Host "[playwright] Running M6 smoke test..."
...
if ($playwrightExit -eq 0) {
    Write-Host "[playwright] M6 smoke test PASSED (exit 0)"
} else {
    Write-Host "[playwright] M6 smoke test FAILED (exit $playwrightExit)"
}

# --- Stop servers ---
Write-Host ""
Write-Host "[harness] Stopping M6 dev servers..."
$stopScript = Join-Path $scriptDir "stop-dev-m6.ps1"
& $stopScript

# --- Report ---
Write-Host ""
Write-Host "=== GlyphDeck M6 Usage Tab Smoke Test Complete ==="
if ($playwrightExit -eq 0) {
