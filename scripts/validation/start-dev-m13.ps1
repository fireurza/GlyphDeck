# GlyphDeck validation harness - start M13 release server (binary serves frontend)
...
Write-Host "=== GlyphDeck M13 — Start Release ==="
...
Start-Job -Name "glyphdeck-m13-backend" -ScriptBlock { param($exe,$log,$port) $env:GLYPHDECK_PORT=$port; & $exe >> $log 2>&1 } -Arg $bkExe,$backendLog,$bp | Out-Null
