# GlyphDeck validation harness - start dev servers (M8)
$ErrorActionPreference = "Stop"
$scriptDir = Split-Path -Parent $MyInvocation.MyCommand.Path
$repoRoot = Resolve-Path (Join-Path $scriptDir "..\..")
$valDir = Join-Path $repoRoot ".glyphdeck\validation\m8"
...
$fp = "5173"
Write-Host "=== GlyphDeck M8 — Start Dev ==="
...
Start-Job -Name "glyphdeck-m8-backend" -ScriptBlock { param($R,$L,$P) $env:GLYPHDECK_PORT=$P; Set-Location $R; & go run ./cmd/glyphdeck >> $L 2>&1 } -Arg $repoRoot,$backendLog,$bp | Out-Null
...
Start-Job -Name "glyphdeck-m8-frontend" -ScriptBlock { param($W,$L) Set-Location $W; & npm run dev >> $L 2>&1 } -Arg (Join-Path $repoRoot "web"),$frontendLog | Out-Null
