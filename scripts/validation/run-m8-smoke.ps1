# GlyphDeck Milestone 8 smoke test runner
...
Write-Host "=== GlyphDeck M8 Permissions Smoke ==="
...
Write-Host "[harness] Starting M8 dev servers..."
& (Join-Path $scriptDir "start-dev-m8.ps1")

$cs = Join-Path $scriptDir "m8-smoke.cjs"
$rs = Join-Path $scriptsRuntimeDir "m8-smoke.cjs"
...
& (Join-Path $scriptDir "stop-dev-m8.ps1")

Write-Host ""; if ($exit -eq 0) { Write-Host "Result: PASS" } else { Write-Host "Result: FAIL" }
exit $exit
