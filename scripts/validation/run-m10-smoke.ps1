# GlyphDeck Milestone 10 smoke test runner
$ErrorActionPreference = "Stop"
$scriptDir = Split-Path -Parent $MyInvocation.MyCommand.Path
$repoRoot = Resolve-Path (Join-Path $scriptDir "..\..")
$valDir = Join-Path $repoRoot ".glyphdeck\validation\m10"
...
Write-Host "=== GlyphDeck M10 POC Hardening Smoke ==="
...
Write-Host "[harness] Starting M10 dev servers..."
& (Join-Path $scriptDir "start-dev-m10.ps1")
$cs = Join-Path $scriptDir "m10-smoke.cjs"
$rs = Join-Path $scriptsRuntimeDir "m10-smoke.cjs"
...
& (Join-Path $scriptDir "stop-dev-m10.ps1")
