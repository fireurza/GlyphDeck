# GlyphDeck validation harness — stop dev servers using only recorded PIDs
# Usage: .\scripts\validation\stop-dev.ps1
...
$pidDir = Join-Path $repoRoot ".glyphdeck\validation\m3_5\pids"

Write-Host "=== GlyphDeck Validation Harness — Stop Dev ==="
...
Get-Job -Name "glyphdeck-backend" -ErrorAction SilentlyContinue | Stop-Job -ErrorAction SilentlyContinue
Get-Job -Name "glyphdeck-backend" -ErrorAction SilentlyContinue | Remove-Job -Force -ErrorAction SilentlyContinue
Write-Host "[backend] PowerShell job stopped and removed."
...
# Stop the PowerShell job itself.
Get-Job -Name "glyphdeck-frontend" -ErrorAction SilentlyContinue | Stop-Job -ErrorAction SilentlyContinue
Get-Job -Name "glyphdeck-frontend" -ErrorAction SilentlyContinue | Remove-Job -Force -ErrorAction SilentlyContinue
Write-Host "[frontend] PowerShell job stopped and removed."
